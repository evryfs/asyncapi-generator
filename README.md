...
Native Protobuf generation consumes AsyncAPI schemas that use a native Protobuf `schemaFormat`. Select the `protobuf` generator and provide a model package to generate `.proto` artifacts and Protobuf model APIs. Java models are generated by default. Selecting the Kotlin model type generates the required Java messages together with the official Protobuf Kotlin DSL.
...
<configuration>
    <inputFile>path/to/my/asyncapi_specification.yaml</inputFile>
    <generatorName>protobuf</generatorName>
    <models>
        <packageName>com.example.protobuf</packageName>
        <protobufModelType>kotlin</protobufModelType>
    </models>
    <resourceOutputDirectory>${project.build.directory}/generated-resources/asyncapi</resourceOutputDirectory>
</configuration>
```
...
Omit `protobufModelType` to use the default Java model API. To generate only `.proto` artifacts, omit the model package and enable `schemas.nativeProtobuf` in Maven or Gradle, or pass `--schemas-native-protobuf` to the CLI.

Generated Protobuf model sources are produced by running `protoc` during generation. The `.proto` schema must declare a Java package or Protobuf package, enable `option java_multiple_files = true;`, and contain a top-level message that matches the payload name. The configured model package must match the schema's Java package, or its Protobuf package when `java_package` is omitted. Generated Java sources require `protobuf-java` on the consuming project's compile classpath; Kotlin DSL output additionally requires `protobuf-kotlin`. The generator does not configure Protobuf serializers or deserializers yet; applications still own Kafka runtime wiring.
