...
Message-example instance validation does not change this decision. It consumes the existing Schema Object graph directly so references, recursive schemas, exact numbers, and nested example source paths remain owned by the parser model. Multi Format Schema examples are reported as an explicit capability limitation rather than being converted into a Draft 7 tree. This decision should still be revisited if a library can consume the domain representation without losing source ownership or can replace both conformance and instance checks with one source-aware engine.
