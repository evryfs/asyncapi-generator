# Parsing Strategy (documentation is WIP)

The **Parser** is the entry point of the AsyncAPI generation pipeline. Its responsibility is to transform raw YAML/JSON input into a strongly-typed, traversable **Abstract Syntax Tree (AST)** (represented by the `AsyncApiDocument` model) while maintaining strict tracking of source code locations for error reporting.
