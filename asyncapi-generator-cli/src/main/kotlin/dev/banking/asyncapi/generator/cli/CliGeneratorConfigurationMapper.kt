...
/**
 * CLI-facing values used to assemble the frontend-neutral generator configuration.
 */
internal data class CliGeneratorConfigurationRequest(
    ...
    val modelConfig: CliModelConfiguration? = null,
    val clientConfig: CliClientConfiguration? = null,
)

/**
 * Maps and validates CLI configuration before contract processing starts.
 *
 * Expected behavior is covered by:
 * - `CliGeneratorConfigurationMapperTest`
 */
internal object CliGeneratorConfigurationMapper {
    fun map(request: CliGeneratorConfigurationRequest): GeneratorConfiguration {
        validateCliConfiguration(request)
        ...
        val modelRequest =
            if (request.modelPackage != null || request.modelConfig != null) {
                (request.modelConfig ?: CliModelConfiguration()).toRequest(request.modelPackage)
            } else {
                null
            }
        val configuration =
    ...
    private fun validateCliConfiguration(request: CliGeneratorConfigurationRequest) {
    ...
    private fun validateActivatedOutputs(
        request: CliGeneratorConfigurationRequest,
