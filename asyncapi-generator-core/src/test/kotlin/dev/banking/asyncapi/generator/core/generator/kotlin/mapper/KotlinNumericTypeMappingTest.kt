...
class KotlinNumericTypeMappingTest {
    ...
    fun `map should return Int for standard integer`() {
        ...
        assertEquals("Int", mapper.mapKotlinType("prop", schema))
    }

    @Test
    fun `map should return Long for int64 format`() {
        val schema = Schema(type = "integer", format = "int64")
        assertEquals("Long", mapper.mapKotlinType("prop", schema))
    }
    ...
    fun `map should return Double for standard number`() {
        ...
        assertEquals("Double", mapper.mapKotlinType("prop", schema))
    }
    ...
    fun `map should return BigDecimal if multipleOf is present`() {
        ...
        assertEquals("BigDecimal", mapper.mapKotlinType("prop", schema))
    }
}
