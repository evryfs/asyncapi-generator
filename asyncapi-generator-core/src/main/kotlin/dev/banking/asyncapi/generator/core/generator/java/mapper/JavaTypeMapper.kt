...
class JavaTypeMapper(val context: GeneratorContext) {
    fun mapJavaType(propertyName: String, schema: Schema?): String {
        ...
        return when (schema.type.getPrimaryType()) {
            ...
            "object" -> mapObject(propertyName, schema) ?: "Object"
            else -> "Object"
        }
    }
    ...
    private fun mapInteger(schema: Schema): String {
        ...
        if (minimum != null || maximum != null) {
            ...
            return if (minimumFits && maximumFits) "Integer" else "Long"
        }
        return "Integer"
    }
    ...
    private fun mapArray(
        ...
        val elementType =
            when (items) {
                is SchemaInterface.SchemaInline -> mapJavaType(propertyName, items.schema)
    ...
    private fun mapObject(
        ...
        if (!schema.properties.isNullOrEmpty()) {
            throw IllegalStateException(
                "Java type mapping encountered an inline object with properties at property '$propertyName'. " +
        ...
        val valueType =
            when (val additionalProperties = schema.additionalProperties) {
                null -> "Object"
                is SchemaInterface.BooleanSchema -> if (additionalProperties.value) "Object" else return null
                is SchemaInterface.SchemaInline -> mapJavaType(propertyName + "Value", additionalProperties.schema)
